...
class EVCDeploy(EVCBase):
    ...
    def _prepare_push_flow(self, *args, queue_id=None):
        ...
        if not(in_vlan != new_c_vlan and isinstance(in_vlan, int) and isinstance(new_c_vlan, int)):
            new_action = {"action_type": "push_vlan", "tag_type": "s"} # DELETE ME
        ...
        #if in_vlan not in self.special_cases and new_c_vlan == 0: # LOOK AT ME (NUMBER - UNTAGGED)
    ...
    def _prepare_pop_flow(
        ...
        if in_vlan == 0:
            new_action = {"action_type": "pop_vlan"} # DELETE ME
            flow_mod["actions"].insert(0, new_action) # DELETE ME
